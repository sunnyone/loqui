...
struct _LqGKeyFile
{
  ...
  LqGKeyFileGroup *start_group;
  LqGKeyFileGroup *current_group;
...
typedef struct _LqGKeyFileKeyValuePair LqGKeyFileKeyValuePair;

struct _LqGKeyFileGroup
{
  const gchar *name;  /* NULL for above first group (which will be comments) */

  LqGKeyFileKeyValuePair *comment; /* Special comment that is stuck to the top of a group */
...
struct _LqGKeyFileKeyValuePair
{
...
/* static gint                  find_file_in_data_dirs            (const gchar            *file,
								...
								gchar                ***data_dirs,
								GError                **error); */
static gboolean              lqg_key_file_load_from_fd           (LqGKeyFile               *key_file,
								gint                    fd,
								LqGKeyFileFlags           flags,
								GError                **error);
static GList                *lqg_key_file_lookup_group_node      (LqGKeyFile               *key_file,
			                                        const gchar            *group_name);
static LqGKeyFileGroup        *lqg_key_file_lookup_group           (LqGKeyFile               *key_file,
								const gchar            *group_name);

static GList                *lqg_key_file_lookup_key_value_pair_node  (LqGKeyFile       *key_file,
			                                             LqGKeyFileGroup  *group,
                                                                     const gchar    *key);
static LqGKeyFileKeyValuePair *lqg_key_file_lookup_key_value_pair       (LqGKeyFile       *key_file,
                                                                     LqGKeyFileGroup  *group,
                                                                     const gchar    *key);

static void                  lqg_key_file_remove_group_node          (LqGKeyFile      *key_file,
							  	    GList         *group_node);
static void                  lqg_key_file_remove_key_value_pair_node (LqGKeyFile      *key_file,
                                                                    LqGKeyFileGroup *group,
                                                                    GList         *pair_node);

static void                  lqg_key_file_add_key                (LqGKeyFile               *key_file,
								LqGKeyFileGroup          *group,
								const gchar            *key,
								const gchar            *value);
static void                  lqg_key_file_add_group              (LqGKeyFile               *key_file,
								const gchar            *group_name);
static void                  lqg_key_file_key_value_pair_free    (LqGKeyFileKeyValuePair   *pair);
static gboolean              lqg_key_file_line_is_comment        (const gchar            *line);
static gboolean              lqg_key_file_line_is_group          (const gchar            *line);
static gboolean              lqg_key_file_line_is_key_value_pair (const gchar            *line);
static gchar                *lqg_key_file_parse_value_as_string  (LqGKeyFile               *key_file,
								const gchar            *value,
								GSList                **separators,
								GError                **error);
static gchar                *lqg_key_file_parse_string_as_value  (LqGKeyFile               *key_file,
								const gchar            *string,
								gboolean                escape_separator);
static gint                  lqg_key_file_parse_value_as_integer (LqGKeyFile               *key_file,
								const gchar            *value,
								GError                **error);
static gchar                *lqg_key_file_parse_integer_as_value (LqGKeyFile               *key_file,
								gint                    value);
static gboolean              lqg_key_file_parse_value_as_boolean (LqGKeyFile               *key_file,
								const gchar            *value,
								GError                **error);
static gchar                *lqg_key_file_parse_boolean_as_value (LqGKeyFile               *key_file,
								gboolean                value);
static gchar                *lqg_key_file_parse_value_as_comment (LqGKeyFile               *key_file,
                                                                const gchar            *value);
static gchar                *lqg_key_file_parse_comment_as_value (LqGKeyFile               *key_file,
                                                                const gchar            *comment);
static void                  lqg_key_file_parse_key_value_pair   (LqGKeyFile               *key_file,
								const gchar            *line,
								gsize                   length,
								GError                **error);
static void                  lqg_key_file_parse_comment          (LqGKeyFile               *key_file,
								const gchar            *line,
								gsize                   length,
								GError                **error);
static void                  lqg_key_file_parse_group            (LqGKeyFile               *key_file,
								const gchar            *line,
								gsize                   length,
								GError                **error);
/* static gchar                *key_get_locale                    (const gchar            *key); */
static void                  lqg_key_file_parse_data             (LqGKeyFile               *key_file,
								const gchar            *data,
								gsize                   length,
								GError                **error);
static void                  lqg_key_file_flush_parse_buffer     (LqGKeyFile               *key_file,
								GError                **error);
...
static void
lqg_key_file_init (LqGKeyFile *key_file)
{  
  key_file->current_group = g_new0 (LqGKeyFileGroup, 1);
...
static void
lqg_key_file_clear (LqGKeyFile *key_file)
{
  ...
  while (tmp != NULL)
    {
      group_node = tmp;
      tmp = tmp->next;
      lqg_key_file_remove_group_node (key_file, group_node);
    }
 ...
 * lqg_key_file_new:
 *
 * Creates a new empty #LqGKeyFile object. Use lqg_key_file_load_from_file(),
 * lqg_key_file_load_from_data() or lqg_key_file_load_from_data_dirs() to
 * read an existing key file.
 *
 * Return value: an empty #LqGKeyFile.
 *
 * Since: 2.6
 **/
LqGKeyFile *
lqg_key_file_new (void)
{
  LqGKeyFile *key_file;

  key_file = g_new0 (LqGKeyFile, 1);
  lqg_key_file_init (key_file);

  return key_file;
}

/**
 * lqg_key_file_set_list_separator:
 * @key_file: a #LqGKeyFile 
 ...
 */
void
lqg_key_file_set_list_separator (LqGKeyFile *key_file,
			       gchar     separator)
{
                        ...
                        GError       **error)
{
  ...
  *dirs = data_dirs;
  ...
  if (fd < 0)
    {
      g_set_error (error, LQG_KEY_FILE_ERROR,
...
static gboolean
lqg_key_file_load_from_fd (LqGKeyFile       *key_file,
			 gint            fd,
			 LqGKeyFileFlags   flags,
			 GError        **error)
{
  ...
  do
    {
      ...
      lqg_key_file_parse_data (key_file, 
			     read_buf, bytes_read,
			     &key_file_error);
    }
  ...
  lqg_key_file_flush_parse_buffer (key_file, &key_file_error);
...
/**
 * lqg_key_file_load_from_file:
 * @key_file: an empty #LqGKeyFile struct
 * @file: the path of a filename to load, in the GLib file name encoding
 * @flags: flags from #LqGKeyFileFlags
 * @error: return location for a #GError, or %NULL
 *
 * Loads a key file into an empty #LqGKeyFile structure.
 * If the file could not be loaded then %error is set to 
 * either a #GFileError or #LqGKeyFileError.
 *
 * Return value: %TRUE if a key file could be loaded, %FALSE othewise
 * Since: 2.6
 **/
gboolean
lqg_key_file_load_from_file (LqGKeyFile       *key_file,
			   const gchar    *file,
			   LqGKeyFileFlags   flags,
			   GError        **error)
{
  ...
  lqg_key_file_load_from_fd (key_file, fd, flags, &key_file_error);
  close (fd);
 ...
 * @error: return location for a #GError, or %NULL
 *
 * Loads a key file from memory into an empty #LqGKeyFile structure.  
 * If the object cannot be created then %error is set to a #LqGKeyFileError.
 *
 * Return value: %TRUE if a key file could be loaded, %FALSE othewise
 * Since: 2.6
 **/
gboolean
lqg_key_file_load_from_data (LqGKeyFile       *key_file,
			   const gchar    *data,
			   gsize           length,
			   LqGKeyFileFlags   flags,
			   GError        **error)
{
  ...
  lqg_key_file_parse_data (key_file, data, length, &key_file_error);
...
/**
 * lqg_key_file_load_from_data_dirs:
 * @key_file: an empty #LqGKeyFile struct
 ...
 * set to either a #GFileError or #LqGKeyFileError.
 *
 * Return value: %TRUE if a key file could be loaded, %FALSE othewise
 * Since: 2.6
 **/
#if 0
gboolean
lqg_key_file_load_from_data_dirs (LqGKeyFile       *key_file,
				const gchar    *file,
				gchar         **full_path,
				LqGKeyFileFlags   flags,
				GError        **error)
  ...
  while (*data_dirs != NULL && !found_file)
    {
      ...
      found_file = lqg_key_file_load_from_fd (key_file, fd, flags,
	                                    &key_file_error);
      close (fd);
 ...
 **/
void
lqg_key_file_free (LqGKeyFile *key_file)
{
...
/* If LQG_KEY_FILE_KEEP_TRANSLATIONS is not set, only returns
 * true for locales that match those in g_get_language_names().
 */
#if 0
static gboolean
lqg_key_file_locale_is_interesting (LqGKeyFile    *key_file,
				  const gchar *locale)
{
  const gchar * const * current_locales;
  gsize i;

  if (key_file->flags & LQG_KEY_FILE_KEEP_TRANSLATIONS)
    return TRUE;
...
static void
lqg_key_file_parse_line (LqGKeyFile     *key_file,
		       const gchar  *line,
		       gsize         length,
		       GError      **error)
{
  ...
  if (lqg_key_file_line_is_comment (line_start))
    lqg_key_file_parse_comment (key_file, line, length, &parse_error);
  else if (lqg_key_file_line_is_group (line_start))
    lqg_key_file_parse_group (key_file, line_start,
			    length - (line_start - line),
			    &parse_error);
  else if (lqg_key_file_line_is_key_value_pair (line_start))
    lqg_key_file_parse_key_value_pair (key_file, line_start,
				     length - (line_start - line),
				     &parse_error);
  else
    {
...
static void
lqg_key_file_parse_comment (LqGKeyFile     *key_file,
			  const gchar  *line,
			  gsize         length,
			  GError      **error)
{
  LqGKeyFileKeyValuePair *pair;
  
  if (!(key_file->flags & LQG_KEY_FILE_KEEP_COMMENTS))
    return;
  
  g_assert (key_file->current_group != NULL);

  pair = g_new0 (LqGKeyFileKeyValuePair, 1);
...
static void
lqg_key_file_parse_group (LqGKeyFile     *key_file,
			const gchar  *line,
			gsize         length,
			GError      **error)
{
  ...
  lqg_key_file_add_group (key_file, group_name);
  g_free (group_name);
}

static void
lqg_key_file_parse_key_value_pair (LqGKeyFile     *key_file,
				 const gchar  *line,
				 gsize         length,
				 GError      **error)
{
  ...
  /* originally, this should read locale information, but add all keys regardless of locale */
  lqg_key_file_add_key (key_file, key_file->current_group, key, value);
...
static void
lqg_key_file_parse_data (LqGKeyFile     *key_file,
		       const gchar  *data,
		       gsize         length,
		       GError      **error)
{
      ...
      if (data[i] == '\n')
        {
          ...
          if (key_file->parse_buffer->len > 0)
            lqg_key_file_flush_parse_buffer (key_file, &parse_error);
          else
            lqg_key_file_parse_comment (key_file, "", 1, &parse_error);
...
static void
lqg_key_file_flush_parse_buffer (LqGKeyFile  *key_file,
			       GError   **error)
{
  ...
  if (key_file->parse_buffer->len > 0)
    {
      lqg_key_file_parse_line (key_file, key_file->parse_buffer->str,
 ...
 *   the contents of the #LqGKeyFile 
 *
 * Since: 2.6
 **/
gchar *
lqg_key_file_to_data (LqGKeyFile  *key_file,
		    gsize     *length,
		    GError   **error)
{
  GString *data_string;
  ...
  for (group_node = g_list_last (key_file->groups);
       ...
       group_node = group_node->prev)
    {
      ...
      group = (LqGKeyFileGroup *) group_node->data;
      ...
      for (key_file_node = g_list_last (group->key_value_pairs);
           ...
           key_file_node = key_file_node->prev)
        {
          ...
          pair = (LqGKeyFileKeyValuePair *) key_file_node->data;
 ...
 **/
gchar **
lqg_key_file_get_keys (LqGKeyFile     *key_file,
		     const gchar  *group_name,
		     gsize        *length,
		     GError      **error)
{
  ...
  group = lqg_key_file_lookup_group (key_file, group_name);
  ...
  for (i = 0; i < num_keys; i++)
    {
      ...
      pair = (LqGKeyFileKeyValuePair *) tmp->data;
 ...
 **/
gchar *
lqg_key_file_get_start_group (LqGKeyFile *key_file)
{
  g_return_val_if_fail (key_file != NULL, NULL);
 ...
 **/
gchar **
lqg_key_file_get_groups (LqGKeyFile *key_file,
		       gsize    *length)
{
  GList *group_node;
  gchar **groups;
  ...
  g_assert (((LqGKeyFileGroup *) group_node->data)->name == NULL);
 ...
 * @error is set to #LQG_KEY_FILE_ERROR_KEY_NOT_FOUND.  In the 
 * event that the @group_name cannot be found, %NULL is returned 
 * and @error is set to #LQG_KEY_FILE_ERROR_GROUP_NOT_FOUND.
 *
 ...
 **/
gchar *
lqg_key_file_get_value (LqGKeyFile     *key_file,
		      const gchar  *group_name,
		      const gchar  *key,
		      GError      **error)
{
  ...
  pair = lqg_key_file_lookup_key_value_pair (key_file, group, key);
 ...
 **/
void
lqg_key_file_set_value (LqGKeyFile    *key_file,
		      const gchar *group_name,
		      const gchar *key,
		      const gchar *value)
{
  ...
  if (!group)
    {
      lqg_key_file_add_group (key_file, group_name);
      group = (LqGKeyFileGroup *) key_file->groups->data;

      lqg_key_file_add_key (key_file, group, key, value);
    }
  else
    {
      pair = lqg_key_file_lookup_key_value_pair (key_file, group, key);
 ...
 **/
gchar *
lqg_key_file_get_string (LqGKeyFile     *key_file,
		       const gchar  *group_name,
		       const gchar  *key,
		       GError      **error)
{
  ...
  value = lqg_key_file_get_value (key_file, group_name, key, &key_file_error);
  ...
  string_value = lqg_key_file_parse_value_as_string (key_file, value, NULL,
						   &key_file_error);
  g_free (value);
 ...
 **/
void
lqg_key_file_set_string (LqGKeyFile    *key_file,
		       const gchar *group_name,
		       const gchar *key,
		       const gchar *string)
{
  ...
  value = lqg_key_file_parse_string_as_value (key_file, string, FALSE);
  lqg_key_file_set_value (key_file, group_name, key, value);
  g_free (value);
}

/**
 * lqg_key_file_get_string_list:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 * @key: a key
 ...
 **/
gchar **
lqg_key_file_get_string_list (LqGKeyFile     *key_file,
			    const gchar  *group_name,
			    const gchar  *key,
			    gsize        *length,
			    GError      **error)
{
  ...
  string_value = lqg_key_file_parse_value_as_string (key_file, value, &pieces, &key_file_error);
  g_free (value);
  g_free (string_value);
 ...
 **/
void
lqg_key_file_set_string_list (LqGKeyFile            *key_file,
			    const gchar         *group_name,
			    const gchar         *key,
			    ...
			    gsize                length)
{
  ...
  for (i = 0; list[i] != NULL && i < length; i++)
    {
      ...
      value = lqg_key_file_parse_string_as_value (key_file, list[i], TRUE);
  ...
  lqg_key_file_set_value (key_file, group_name, key, value_list->str);
  g_string_free (value_list, TRUE);
}

/**
 * lqg_key_file_get_boolean:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 * @key: a key
 ...
 * @error is set to #LQG_KEY_FILE_ERROR_KEY_NOT_FOUND. Likewise, if
 ...
 **/
gboolean
lqg_key_file_get_boolean (LqGKeyFile     *key_file,
			const gchar  *group_name,
			const gchar  *key,
			GError      **error)
{
  ...
  bool_value = lqg_key_file_parse_value_as_boolean (key_file, value,
						  &key_file_error);
  g_free (value);
 ...
 **/
void
lqg_key_file_set_boolean (LqGKeyFile    *key_file,
			const gchar *group_name,
			const gchar *key,
			gboolean     value)
{
  ...
  result = lqg_key_file_parse_boolean_as_value (key_file, value);
  lqg_key_file_set_value (key_file, group_name, key, result);
  g_free (result);
}

/**
 * lqg_key_file_get_boolean_list:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 * @key: a key
 ...
 **/
gboolean *
lqg_key_file_get_boolean_list (LqGKeyFile     *key_file,
			     const gchar  *group_name,
			     const gchar  *key,
			     gsize        *length,
			     GError      **error)
{
  ...
  values = lqg_key_file_get_string_list (key_file, group_name, key,
  ...
  for (i = 0; i < num_bools; i++)
    {
      bool_values[i] = lqg_key_file_parse_value_as_boolean (key_file,
							  values[i],
							  &key_file_error);
 ...
 **/
void
lqg_key_file_set_boolean_list (LqGKeyFile    *key_file,
			     const gchar *group_name,
			     const gchar *key,
			     gboolean     list[],
			     gsize        length)
  ...
  for (i = 0; i < length; i++)
    {
      ...
      value = lqg_key_file_parse_boolean_as_value (key_file, list[i]);
 ...
 **/
gint
lqg_key_file_get_integer (LqGKeyFile     *key_file,
			const gchar  *group_name,
			const gchar  *key,
			GError      **error)
{
  ...
  int_value = lqg_key_file_parse_value_as_integer (key_file, value,
						 &key_file_error);
  g_free (value);
 ...
 **/
void
lqg_key_file_set_integer (LqGKeyFile    *key_file,
			const gchar *group_name,
			const gchar *key,
			gint         value)
{
  ...
  result = lqg_key_file_parse_integer_as_value (key_file, value);
  lqg_key_file_set_value (key_file, group_name, key, result);
  g_free (result);
}

/**
 * lqg_key_file_get_integer_list:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 * @key: a key
 ...
 **/
gint *
lqg_key_file_get_integer_list (LqGKeyFile     *key_file,
			     const gchar  *group_name,
			     const gchar  *key,
			     gsize        *length,
			     GError      **error)
{
  ...
  for (i = 0; i < num_ints; i++)
    {
      int_values[i] = lqg_key_file_parse_value_as_integer (key_file,
							 values[i],
							 &key_file_error);
 ...
 **/
void
lqg_key_file_set_integer_list (LqGKeyFile     *key_file,
			     const gchar  *group_name,
			     const gchar  *key,
			     gint          list[],
			     gsize         length)
{
  ...
  for (i = 0; i < length; i++)
    {
      ...
      value = lqg_key_file_parse_integer_as_value (key_file, list[i]);
  ...
  lqg_key_file_set_value (key_file, group_name, key, values->str);
  g_string_free (values, TRUE);
}

static void
lqg_key_file_set_key_comment (LqGKeyFile             *key_file,
                            const gchar          *group_name,
                            const gchar          *key,
                            const gchar          *comment,
                            GError              **error)
{
   ...
   * associated with
   */
  key_node = lqg_key_file_lookup_key_value_pair_node (key_file, group, key);
  ...
  while (tmp != NULL)
    {
      ...
      comment_node = tmp;
      tmp = tmp->next;
      lqg_key_file_remove_key_value_pair_node (key_file, group,
                                             comment_node); 
    }
  ...
  pair->key = NULL;
  pair->value = lqg_key_file_parse_comment_as_value (key_file, comment);
  
  key_node = g_list_insert (key_node, pair, 1);
}

static void
lqg_key_file_set_group_comment (LqGKeyFile             *key_file,
                              const gchar          *group_name,
                              const gchar          *comment,
                              GError              **error)
{
  ...
  if (group->comment)
    {
      lqg_key_file_key_value_pair_free (group->comment);
      group->comment = NULL;
    }
  ...
  /* Now we can add our new comment
   */
  group->comment = g_new0 (LqGKeyFileKeyValuePair, 1);
  
  group->comment->key = NULL;
  group->comment->value = lqg_key_file_parse_comment_as_value (key_file, comment);
}

static void
lqg_key_file_set_top_comment (LqGKeyFile             *key_file,
                            const gchar          *comment,
                            GError              **error)
{
  GList *group_node;
  ...
  if (group->key_value_pairs != NULL)
    {
      g_list_foreach (group->key_value_pairs, 
                      (GFunc) lqg_key_file_key_value_pair_free, 
                      NULL);
 ...
 **/
void
lqg_key_file_set_comment (LqGKeyFile             *key_file,
                        const gchar          *group_name,
                        const gchar          *key,
                        const gchar          *comment,
                        GError              **error)
{
  ...
  if (group_name != NULL && key != NULL)
    lqg_key_file_set_key_comment (key_file, group_name, key, comment, error);
  else if (group_name != NULL)
    lqg_key_file_set_group_comment (key_file, group_name, comment, error);
  else
    lqg_key_file_set_top_comment (key_file, comment, error);
...
static gchar *
lqg_key_file_get_key_comment (LqGKeyFile             *key_file,
                            const gchar          *group_name,
                            const gchar          *key,
                            GError              **error)
{
  ...
  while (tmp != NULL)
    {
      ...
      comment = lqg_key_file_parse_value_as_comment (key_file, pair->value);
...
static gchar *
lqg_key_file_get_group_comment (LqGKeyFile             *key_file,
                              const gchar          *group_name,
                              GError              **error)
{
  LqGKeyFileGroup *group;
...
static gchar *
lqg_key_file_get_top_comment (LqGKeyFile             *key_file,
                            GError              **error)
{
 ...
 **/
gchar * 
lqg_key_file_get_comment (LqGKeyFile             *key_file,
                        const gchar          *group_name,
                        const gchar          *key,
                        GError              **error)
{
  ...
  if (group_name != NULL && key != NULL)
    return lqg_key_file_get_key_comment (key_file, group_name, key, error);
  else if (group_name != NULL)
    return lqg_key_file_get_group_comment (key_file, group_name, error);
  else
    return lqg_key_file_get_top_comment (key_file, error);
}

/**
 * lqg_key_file_remove_comment:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name, or %NULL
 * @key: a key
 * @error: return location for a #GError
...
void
lqg_key_file_remove_comment (LqGKeyFile             *key_file,
                           const gchar          *group_name,
                           const gchar          *key,
                           GError              **error)
{
  ...
  if (group_name != NULL && key != NULL)
    lqg_key_file_set_key_comment (key_file, group_name, key, NULL, error);
  else if (group_name != NULL)
    lqg_key_file_set_group_comment (key_file, group_name, NULL, error);
  else
    lqg_key_file_set_top_comment (key_file, NULL, error);
}

/**
 * lqg_key_file_has_group:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 *
 ...
 **/
gboolean
lqg_key_file_has_group (LqGKeyFile    *key_file,
		      const gchar *group_name)
{
  g_return_val_if_fail (key_file != NULL, FALSE);
  g_return_val_if_fail (group_name != NULL, FALSE);

  return lqg_key_file_lookup_group_node (key_file, group_name) != NULL;
}

/**
 * lqg_key_file_has_key:
 * @key_file: a #LqGKeyFile
 * @group_name: a group name
 * @key: a key name
 * @error: return location for a #GError
 ...
 **/
gboolean
lqg_key_file_has_key (LqGKeyFile     *key_file,
		    const gchar  *group_name,
		    const gchar  *key,
		    GError      **error)
{
...
static void
lqg_key_file_add_group (LqGKeyFile    *key_file,
		      const gchar *group_name)
{
  LqGKeyFileGroup *group;
  ...
  g_return_if_fail (key_file != NULL);
  g_return_if_fail (group_name != NULL);
  g_return_if_fail (lqg_key_file_lookup_group_node (key_file, group_name) == NULL);

  group = g_new0 (LqGKeyFileGroup, 1);
...
static void
lqg_key_file_key_value_pair_free (LqGKeyFileKeyValuePair *pair)
{
  if (pair != NULL)
    {
      g_free (pair->key);
 ...
 */ 
static void
lqg_key_file_remove_key_value_pair_node (LqGKeyFile      *key_file,
                                       LqGKeyFileGroup *group,
			               GList         *pair_node)
{
  ...
  pair = (LqGKeyFileKeyValuePair *) pair_node->data;
  ...
  lqg_key_file_key_value_pair_free (pair);

  g_list_free_1 (pair_node);
}

static void
lqg_key_file_remove_group_node (LqGKeyFile *key_file,
			      GList    *group_node)
{
  LqGKeyFileGroup *group;
  GList *tmp;
  ...
  if (key_file->current_group == group)
    {
       ...
       * unless lqg_key_file_clear has been called
       */
      if (key_file->groups)
        key_file->current_group = (LqGKeyFileGroup *) key_file->groups->data;
      else
  ...
  if (key_file->start_group == group)
	...
	{
	  if (tmp != group_node &&
	      ((LqGKeyFileGroup *) tmp->data)->name != NULL)
	    break;
      ...
      if (tmp)
        key_file->start_group = (LqGKeyFileGroup *) tmp->data;
      else
        key_file->start_group = NULL;
    }
  ...
  while (tmp != NULL)
    {
      ...
      pair_node = tmp;
      tmp = tmp->next;
      lqg_key_file_remove_key_value_pair_node (key_file, group, pair_node);
    }
 ...
 **/
void
lqg_key_file_remove_group (LqGKeyFile     *key_file,
			 const gchar  *group_name,
			 GError      **error)
{
  GList *group_node;
  ...
  group_node = lqg_key_file_lookup_group_node (key_file, group_name);
...
static void
lqg_key_file_add_key (LqGKeyFile      *key_file,
		    LqGKeyFileGroup *group,
		    const gchar   *key,
		    const gchar   *value)
{
 ...
 **/
void
lqg_key_file_remove_key (LqGKeyFile     *key_file,
		       const gchar  *group_name,
		       const gchar  *key,
		       GError      **error)
{
...
static GList *
lqg_key_file_lookup_group_node (LqGKeyFile    *key_file,
			      const gchar *group_name)
{
  LqGKeyFileGroup *group;
  GList *tmp;
  ...
  for (tmp = key_file->groups; tmp != NULL; tmp = tmp->next)
    {
      group = (LqGKeyFileGroup *) tmp->data;
...
static LqGKeyFileGroup *
lqg_key_file_lookup_group (LqGKeyFile    *key_file,
			 const gchar *group_name)
{
  GList *group_node;
  ...
  if (group_node != NULL)
    return (LqGKeyFileGroup *) group_node->data; 

  return NULL;
}

static GList *
lqg_key_file_lookup_key_value_pair_node (LqGKeyFile       *key_file,
			               LqGKeyFileGroup  *group,
                                       const gchar    *key)
{
  GList *key_node;
  ...
  for (key_node = group->key_value_pairs;
       ...
       key_node = key_node->next)
    {
      ...
      pair = (LqGKeyFileKeyValuePair *) key_node->data; 
...
static LqGKeyFileKeyValuePair *
lqg_key_file_lookup_key_value_pair (LqGKeyFile      *key_file,
				  LqGKeyFileGroup *group,
				  const gchar   *key)
{
  return (LqGKeyFileKeyValuePair *) g_hash_table_lookup (group->lookup_map, key);
}
 ...
 */
static gboolean
lqg_key_file_line_is_comment (const gchar *line)
{
 ...
 */
static gboolean
lqg_key_file_line_is_group (const gchar *line)
{
  gchar *p;
...
static gboolean
lqg_key_file_line_is_key_value_pair (const gchar *line)
{
  gchar *p;
...
static gchar *
lqg_key_file_parse_value_as_string (LqGKeyFile     *key_file,
				  const gchar  *value,
				  GSList      **pieces,
				  GError      **error)
{
...
static gchar *
lqg_key_file_parse_string_as_value (LqGKeyFile    *key_file,
				  const gchar *string,
				  gboolean     escape_separator)
{
  gchar *value, *p, *q;
...
static gint
lqg_key_file_parse_value_as_integer (LqGKeyFile     *key_file,
				   const gchar  *value,
				   GError      **error)
{
...
static gchar *
lqg_key_file_parse_integer_as_value (LqGKeyFile *key_file,
				   gint      value)
...
static gboolean
lqg_key_file_parse_value_as_boolean (LqGKeyFile     *key_file,
				   const gchar  *value,
				   GError      **error)
{
  if (value)
...
static gchar *
lqg_key_file_parse_boolean_as_value (LqGKeyFile *key_file,
				   gboolean  value)
{
  if (value)
...
static gchar *
lqg_key_file_parse_value_as_comment (LqGKeyFile    *key_file,
                                   const gchar *value)
{
  GString *string;
  gchar **lines, *comment;
...
static gchar *
lqg_key_file_parse_comment_as_value (LqGKeyFile      *key_file,
                                   const gchar   *comment)
{
  GString *string;
  gchar **lines, *value;
