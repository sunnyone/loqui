...
eel_gconf_client_get_global (void)
{
	...
	if (!gconf_is_initialized ()) {
		...
		testkey = gnome_gconf_get_app_settings_relative(NULL, "gconf_test");
		...
		{	
			...
			dialog = gtk_message_dialog_new (NULL,
							 ...
							 GTK_MESSAGE_ERROR,
							 GTK_BUTTONS_OK,
							 _("Cannot find a schema for loqui preferences. \n"
						         "Check your gconf setup"));
 ...
 * so that a call to galeon_notification_free can remove the notifiers
 */
void 
galeon_notification_add (const char *key,
			 GConfClientNotifyFunc notification_callback,
			 gpointer callback_data, 
			 GList **notifiers)
{
...
void
galeon_notification_remove(GList **notifiers)
{
...
/* added by Loqui */
GSList*
eel_gconf_get_dirs(const char *key)
{
	GConfClient *client;
	GError *error;
	GSList *list;
	...
	error = NULL;
	list = gconf_client_all_dirs(client, key, &error);
	eel_gconf_handle_error (&error);
