...
static void gtk_separator_tool_item_finalize     (GObject *object);


static GObjectClass *parent_class = NULL;

#define GTK_SEPARATOR_TOOL_ITEM_GET_PRIVATE(obj)(GTK_SEPARATOR_TOOL_ITEM(obj)->private_data)
...
{
  gint space_size = _gtk24_toolbar_get_default_space_size();
  GtkWidget *parent = GTK_WIDGET (tool_item)->parent;
  
  if (GTK24_IS_TOOLBAR (parent))
    {
...
gtk_separator_tool_item_init (GtkSeparatorToolItem      *separator_item,
			      GtkSeparatorToolItemClass *class)
{
  separator_item->priv = g_new0(GtkSeparatorToolItemPrivate, 1);
...
gtk_separator_tool_item_expose (GtkWidget      *widget,
				GdkEventExpose *event)
{
  Gtk24Toolbar *toolbar = NULL;

  if (widget->parent && GTK24_IS_TOOLBAR (widget->parent))
    toolbar = GTK24_TOOLBAR (widget->parent);

  _gtk24_toolbar_paint_space_line (widget, toolbar,
