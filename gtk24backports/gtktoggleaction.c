...
static void gtk_toggle_action_finalize     (GObject *object);
...
gtk_toggle_action_class_init (GtkToggleActionClass *klass)
{
  ...
  gobject_class->finalize = gtk_toggle_action_finalize;
...
{
  action->private_data = g_new0(GtkToggleActionPrivate, 1);
  action->private_data->active = FALSE;
  action->private_data->draw_as_radio = FALSE;
}

static void
gtk_toggle_action_finalize (GObject *object)
{
  GtkToggleAction *action;

  action = GTK_TOGGLE_ACTION (object);
