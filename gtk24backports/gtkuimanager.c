...
#define GTK_UI_MANAGER_GET_PRIVATE(obj) (GTK_UI_MANAGER(obj)->private_data)
...
gtk_ui_manager_class_init (GtkUIManagerClass *klass)
{
  ...
  ui_manager_signals[CONNECT_PROXY] =
    g_signal_new ("connect_proxy",
		  ...
		  NULL, NULL,
		  _gtk24_marshal_VOID__OBJECT_OBJECT,
		  G_TYPE_NONE, 2, 
...
gtk_ui_manager_init (GtkUIManager *self)
{
  ...
  self->private_data = g_new0(GtkUIManagerPrivate, 1);
		       ...
		       gint       *pos_p)
{
  ...
  if (node->prev == NULL)
	...
	case NODE_TYPE_TOOLBAR_PLACEHOLDER:
	  ...
	  pos = gtk24_toolbar_get_item_index (GTK24_TOOLBAR (toolbar),
...
update_smart_separators (GtkWidget *proxy)
{
  ...
  if (GTK_IS_MENU (proxy) || GTK24_IS_TOOLBAR (proxy))
    parent = proxy;
	...
	case NODE_TYPE_TOOLBAR_PLACEHOLDER:
	  ...
	  if (info->proxy == NULL)
	      ...
	      if (find_toolbar_position (node, &toolbar, &pos))
		{
		  ...
		  gtk24_toolbar_insert (GTK24_TOOLBAR (toolbar), item, pos);
		  ...
		  item = gtk_separator_tool_item_new ();
		  gtk24_toolbar_insert (GTK24_TOOLBAR (toolbar), item, pos+1);
	...
	case NODE_TYPE_TOOLITEM:
	  ...
	  if (info->proxy == NULL)
	      ...
	      if (find_toolbar_position (node, &toolbar, &pos))
		{
		  ...
		  gtk24_toolbar_insert (GTK24_TOOLBAR (toolbar),
